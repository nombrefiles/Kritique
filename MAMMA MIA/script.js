...
function calculaMedia() {
    notas = [];
    for (let e of data) {
        if (typeof e.mmm === 'number' && !isNaN(e.mmm)) {
            notas.push(e.mmm);
        }
    }
...
function atualizarNotaUsuarioLogado(nota) {
    ...
    if (usuarioIndex !== -1) {
        data[usuarioIndex].mmm = nota;
    }
    localStorage.setItem('data', JSON.stringify(data));
}

if (usuarioLogado && typeof usuarioLogado.mmm === 'number' && !isNaN(usuarioLogado.mmm)) {
    flag = true;
    input.value = usuarioLogado.mmm;
    notas.push(usuarioLogado.mmm);
