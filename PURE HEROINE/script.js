...
function calculaMedia() {
    notas = [];
    for (let e of data) {
        if (typeof e.phr === 'number' && !isNaN(e.phr)) {
            notas.push(e.phr);
        }
    }
...
function atualizarNotaUsuarioLogado(nota) {
    ...
    if (usuarioIndex !== -1) {
        data[usuarioIndex].phr = nota;
    }
    localStorage.setItem('data', JSON.stringify(data));
}

if (usuarioLogado && typeof usuarioLogado.phr === 'number' && !isNaN(usuarioLogado.phr)) {
    flag = true;
    input.value = usuarioLogado.phr;
    notas.push(usuarioLogado.phr);
