...
function calculaMedia() {
    notas = [];
    for (let e of data) {
        if (typeof e.fwn === 'number' && !isNaN(e.fwn)) {
            notas.push(e.fwn);
        }
    }
...
function atualizarNotaUsuarioLogado(nota) {
    ...
    if (usuarioIndex !== -1) {
        data[usuarioIndex].fwn = nota;
    }
    localStorage.setItem('data', JSON.stringify(data));
}

if (usuarioLogado && typeof usuarioLogado.fwn === 'number' && !isNaN(usuarioLogado.fwn)) {
    flag = true;
    input.value = usuarioLogado.fwn;
    notas.push(usuarioLogado.fwn);
