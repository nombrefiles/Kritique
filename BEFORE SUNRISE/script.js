...
function calculaMedia() {
    notas = [];
    for (let e of data) {
        if (typeof e.bef === 'number' && !isNaN(e.bef)) {
            notas.push(e.bef);
        }
    }
...
function atualizarNotaUsuarioLogado(nota) {
    ...
    if (usuarioIndex !== -1) {
        data[usuarioIndex].bef = nota;
    }
    localStorage.setItem('data', JSON.stringify(data));
}

if (usuarioLogado && typeof usuarioLogado.bef === 'number' && !isNaN(usuarioLogado.bef)) {
    flag = true;
    input.value = usuarioLogado.bef;
    notas.push(usuarioLogado.bef);
