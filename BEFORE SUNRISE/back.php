...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "SELECT * FROM notas_filmes WHERE filme_id = ? AND usuario_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$filmeId, $usuarioLogado["id"]]);
    $notaUser = $stmt->fetch();

    if ($notaUser) {
        $sql = "UPDATE notas_filmes SET nota = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$input, $notaUser['id']]);
        $message = "Nota atualizada com sucesso";
    } else {
        $sql = "INSERT INTO notas_filmes (filme_id, usuario_id, nota) VALUES (?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$filmeId, $usuarioLogado["id"], $input]);
        $message = "Nota adicionada com sucesso";
    }
}

$sql = "SELECT nota FROM notas_filmes WHERE filme_id = ?";
$stmt = $conn->prepare($sql);
...
$sql = "SELECT nota FROM notas_filmes WHERE filme_id = ? AND usuario_id = ?";
$stmt = $conn->prepare($sql);
$stmt->execute([$filmeId, $usuarioLogado["id"]]);
